...
self.createReceipt = async (req, res) => {  
  ...
  try {
    ...
    const createdBill = await createBill(cuit, sellPoint, receiptType, nextBillNumber, amount, req);
...
self.createCreditNote = async (req, res) => {  
  try {
    ...
    console.log('related receipt number pre', relatedReceipt.number)
    relatedReceipt.number = parseBillNumber(relatedReceipt.number)
    console.log('related receipt number post', relatedReceipt.number)
...
async function createBill(cuit, sellPoint, receiptType, nextBillNumber, amount, req) {
  ...
  return await afip.createBill(
    buildBillBody(cuit, sellPoint, receiptType, concept, nextBillNumber, ImpTotal, ImpTotConc, ImpNeto, ImpOpEx, ImpIVA, relatedReceipt)
  );
}
...
function buildBillBody(cuit, sellPoint, receiptType, concept, nextBillNumber, ImpTotal, ImpTotConc, ImpNeto, ImpOpEx, ImpIVA, relatedReceipt) {
  const body = {
    Auth: {
      Cuit: cuit
    },
